...
@section('title', 'Edit Penyemprotan')
@section('header', 'Edit Penyemprotan')
...
@section('content')
    <div class="rounded-lg bg-white p-6 shadow-md">
        ...
        <form action="{{ route('spraying.update', ['spraying' => $spraying->id]) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="col-span-1 grid grid-cols-1 gap-6 md:col-span-2 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            id="land-area" name="land_area" type="text" value="{{ $spraying->land->land_area }}"
                            required>
                        @error('land_area')
                    ...
                    <div>
                        ...
                        <select
                            ...
                            <option value="Afdeling 1" @selected($spraying->land->land_location == 'Afdeling 1')>Afdeling 1</option>
                            <option value="Afdeling 2" @selected($spraying->land->land_location == 'Afdeling 2')>Afdeling 2</option>
                            <option value="Afdeling 3" @selected($spraying->land->land_location == 'Afdeling 3')>Afdeling 3</option>
                            <option value="Afdeling 4" @selected($spraying->land->land_location == 'Afdeling 4')>Afdeling 4</option>
                            <option value="Afdeling 5" @selected($spraying->land->land_location == 'Afdeling 5')>Afdeling 5</option>
                        </select>
                        @error('land_location')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                value="{{ $spraying->land->planting_year }}" step="0.01" min="0" required>
                        </div>
                        @error('planting_year')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="col-span-1 grid grid-cols-1 gap-6 md:col-span-2 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                id="amount-spraying" name="amount_spraying" type="number"
                                value="{{ $spraying->amount_spraying }}" required>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            value="{{ date('Y-m-d', strtotime($spraying->spraying_date)) }}">
                        @error('spraying_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="w-full rounded-md border border-gray-300 pr-2">
                            <select
                                ...
                                @foreach ($pesticideStocks as $pesticideStock)
                                    <option value="{{ $pesticideStock->id }}" @selected($spraying->pesticide->id == $pesticideStock->id)>
                                        {{ $pesticideStock->name }}
                                    </option>
                                @endforeach
                            </select>
