            ...
            <form class="flex w-full flex-col gap-3 sm:flex-row md:w-auto" action="{{ route('fertilization.index') }}"
                ...
                <label class="rounded-md border border-gray-300 pr-2" for="sort-by">
                    <select
                        ...
                        <option value="fertilization_date" @selected(request()->get('sortBy') == 'fertilization_date')>Tanggal Pembabatan</option>
                        <option value="amount_fertilized" @selected(request()->get('sortBy') == 'amount_fertilized')>Jumlah Pembabatan</option>
                    </select>
                </label>
            </form>
                            ...
                            <td class="whitespace-nowrap text-right text-sm font-medium">
                                <div class="relative inline-block text-left">
                                    ...
                                    <div class="dropdown-menu mt-2 w-48 rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5"
                                        ...
                                        <form
                                            action="{{ route('prunning.destroy', ['prunning' => $prunning->id]) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button
